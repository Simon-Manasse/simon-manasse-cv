...
const Hobby = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="w-[25rem]"><div class="h-[15rem] flex justify-center items-center"> <img class="h-[15rem]" src="${"img/hobbies/" + escape(imageName, true)}" alt=""></div> <div class="flex justify-center items-center"><p class="text-4xl py-4">${escape(projectName)}</p></div> <p class="">${escape(description)}</p></div>`;
});
const Welcome = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<section class="ScrollPics " id="Home" data-svelte-h="svelte-wdftkt"> <div class="SC_inner " id="section_0"><figure></figure> <img id="logo" class="h-[20%] flex fixed right-[45%] top-[5rem]" src="img/canvalogo.png" alt=""> <h1 class="SC_title z-10 text-[6rem] text-white items-center justify-center">Welcome to my webpage</h1> <hr id="welcomPageLine" class="SC_content mt-[34rem] w-[60rem] left-[50%] ml-[-30rem] h-[3px] margin-auto z-[100] bg-gradient-to-r from-black via-white to-black border-0 rounded-full "> <p class="SC_content justify-center items-center text-4xl mt-[20rem]">Created by Simon Manassé</p></div></section>`;
});
...
const EducationCards = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div${add_attribute("id", id, 0)} class="${escape(classes, true) + " SC_content perspectiveDiv h-[40rem] w-[30rem] bg-white text-black flex flex-col"}"><img src="${"img/" + escape(image, true)}" class="mt-5 h-[10rem]" alt=""> <div class="items-center flex flex-col my-7 text-xl "><h1 class="justify-center items-center">${escape(title)}</h1></div> <p class="text-justify">${escape(description)}</p></div>`;
});
